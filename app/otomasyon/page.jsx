...
function OtomasyonApp() {
  const searchParams = useSearchParams();
  const { companies } = useCompanyList();
  const { isAdmin } = useAdminAccess();

  const [view, setView] = useState(searchParams.get("view") || "flows");
    ...
    <div className="min-h-screen bg-[#050816] p-6 text-white">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-6 flex flex-wrap gap-2">
          {N8N_AUTOMATION_VIEWS.map((item) => (
            <button
              key={item.id}
              type="button"
              onClick={() => setView(item.id)}
              className={`rounded-lg px-4 py-2 text-sm ${
                view === item.id ? "bg-orange-600" : "bg-slate-800 hover:bg-slate-700"
              }`}
            >
              {item.label}
            </button>
